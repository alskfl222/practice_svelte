<script lang="ts">
	...
	import { dragStart, dragDrop, touchStart, touchMove, touchEnd } from '$utils/calendar';
		...
		<div class="grid grid-cols-1 md:grid-cols-2 gap-2">
			{#each dayObj[dayType] as day}
				...
				>
					...
					<div class="min-h-[60px] px-2 ss:px-4 flex flex-col justify-center gap-1 rounded-2xl">
						{#if $calendarData[day].length > 0}
							{#each getDayCharsArr($calendarData[day]) as charArr}
								...
								>
									<div class="p-1 xs:px-4 flex justify-between">
									...
									{#if selected.day === day && selected.char === charArr[0].char.name}
										<div class="p-1 grid grid-cols-1 xs:grid-cols-2 gap-2 sm:gap-4">
											{#each charArr as item}
												<div
													draggable="true"
													class="aspect-video p-1 flex flex-col justify-end items-center
													...
													on:touchstart|stopPropagation={(e) => touchStart(e, [item])}
													on:touchmove={(e) => touchMove(e)}
													on:touchend={(e) => touchEnd(e)}
												>
							...
							{/each}
						{:else}
							<div class="py-2 flex justify-center items-center border rounded-xl">
